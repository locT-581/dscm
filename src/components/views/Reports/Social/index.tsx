...
export default function SocialReport() {
    ...
    const loadBlockchainData = async () => {
      ...
      if (networkData) {
        ...
        for (let i = 1; i <= socialCount; i++) {
          ...
          setFormAccount((accounts) => [...accounts, newSocial.account]);
  ...
  useEffect(() => {
    const charts = async () => {
      ...
      const emp = data.map((a) => parseInt(a.document.emp));
      const trainh = data.map((a) => parseInt(a.document.trainh) / parseInt(a.document.trainemp));
      const trainemp = data.map((a) => parseInt(a.document.trainemp));
      const resemp = data.map((a) => parseInt(a.document.resemp));
      const hiredemp = data.map((a) => parseInt(a.document.hiredemp));
      const fullemp = data.map((a) => parseInt(a.document.fullemp));
      const partemp = data.map((a) => parseInt(a.document.emp) - parseInt(a.document.fullemp));
      const workh = data.map((a) => parseInt(a.document.workh));
      const overtimeh = data.map((a) => parseInt(a.document.overtimeh));
      const empwage = data.map((a) => parseInt(a.document.empwage));
      const minwage = data.map((a) => parseInt(a.document.minwage));
      const insurance = data.map((a) => parseInt(a.document.insurance));
      const femwage = data.map((a) => parseInt(a.document.femwage));
      const malwage = data.map((a) => parseInt(a.document.malwage));
      const fem = data.map((a) => parseInt(a.document.fem));
      const male = data.map((a) => parseInt(a.document.male));
      const femboard = data.map((a) => parseInt(a.document.femboard));
      const empboard = data.map((a) => parseInt(a.document.empboard));
      const disabled = data.map((a) => parseInt(a.document.disabled));
      const minority = data.map((a) => parseInt(a.document.minority));
      const older = data.map((a) => parseInt(a.document.older));
      const workacc = data.map((a) => parseInt(a.document.minority));
      const empunion = data.map((a) => parseInt(a.document.empunion));
      const bargain = data.map((a) => parseInt(a.document.bargain));
      const discri = data.map((a) => parseInt(a.document.discri));
      const child = data.map((a) => parseInt(a.document.child));
      const forced = data.map((a) => parseInt(a.document.forced));
      const indig = data.map((a) => parseInt(a.document.indig));
      const localemp = data.map((a) => parseInt(a.document.localemp));
      const localsup = data.map((a) => parseInt(a.document.localsup));
      const donation = data.map((a) => parseInt(a.document.donation));
      const earning = data.map((a) => parseInt(a.document.earning));
      const corrup = data.map((a) => parseInt(a.document.corrup));
      const anticomp = data.map((a) => parseInt(a.document.anticomp));
      const socialsus = data.map((a) => parseInt(a.document.socialsus));
      const productassess = data.map((a) => parseInt(a.document.productassess));
      const product = data.map((a) => parseInt(a.document.product));
      const productincident = data.map((a) => parseInt(a.document.productincident));
      const privacy = data.map((a) => parseInt(a.document.privacy));
      const leaks = data.map((a) => parseInt(a.document.leaks));
      const cuscomp = data.map((a) => parseInt(a.document.cuscomp));
      const suppliers = data.map((a) => parseInt(a.document.suppliers));
