...
export default function SocialForm() {
  ...
  const [enviros, setEnviros] = useState<Enviro[]>([]);
  ...
  const [solidwasterec, setSolidwasterec] = useState<number>(0);
  ...
  useEffect(() => {
    const loadBlockchainData = async () => {
      ...
      if (networkData) {
        ...
        setEnviroCount(Number(enviroCount));
        //Load Enviros
        for (let i = 1; i <= enviroCount; i++) {
          ...
          setEnviros((enviros) => [
            ...enviros,
            {
              ...
              document: JSON.parse(newEnviro.document) as unknown as Document,
            } as Enviro,
          ]);
        }
      } else {
  ...
  const getMonth = () => {
    const month = monthYear.toString().slice(-2);
    const year = monthYear.toString().slice(0, 4);
    setYear(year);
    for (let i = 0; i <= 11; i++) {
  ...
  const handleChange = (name: string, checked: boolean, set: React.Dispatch<React.SetStateAction<string[]>>) => {
    if (checked) {
      ...
      set((prev) => prev.filter((item) => item !== name));
    }
  };
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const enviroForm = {
      id: (parseInt(enviroCount.toString()) + 1).toString(),
      energy: energy,
